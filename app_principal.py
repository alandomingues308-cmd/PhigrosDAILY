...
nuevo_usuario = col_user1.text_input("Nombre de usuario", 
                                     value=st.session_state.usuario_actual, 
                                     key="user_input")

if col_user2.button("Cambiar"):
    if nuevo_usuario.strip():
        st.session_state.usuario_actual = nuevo_usuario.strip()
        st.sidebar.success(f"Usuario cambiado a: **{nuevo_usuario}**")
    else:
        st.sidebar.warning("Ingresa un nombre de usuario")
...
with tab_phigros:
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Analizando captura..."):
            ...
            # Correcciones
            if score_detectado == 10000: 
                score_detectado = 1000000
            if score_detectado == 100000: 
                score_detectado = 1000000
            if score_detectado == 1000000: 
                accuracy_detectada = 100
